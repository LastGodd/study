...
public class Main {
	...
	public static void main(String[] args) {
		...
		// 배열 항목 비교
		// - equals : 1차 항목의 값만 비교
		// - deepEquals : 1차 항목이 서로 다른 배열을 참고할 경우 중첩된 배열의 항목까지 비교
		...
		/*
		 * 배열 항목 정렬 - 기본 타입 또는 String 타입일 경우엔 자동으로 오름차순 정렬 - 사용자 정의 클래스 타입일 경우에는 클래스가
		...
		Arrays.sort(scores1);
		int idx = Arrays.binarySearch(scores1, 99);
		System.out.println("찾은 인덱스 : " + idx);
		
		String[] names1 = { "홍길동", "박동수", "김민수" };
		Arrays.sort(names1);
		idx = Arrays.binarySearch(names1, "홍길동");
		System.out.println("찾은 인덱스 : " + idx);
		...
		idx = Arrays.binarySearch(MemberList, m6);
		System.out.println("찾은 인덱스 : " + idx);
